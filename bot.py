...
def get_next_signal_ids(count=1):
    ...
    next_ids = [f"{current_id + i + 1:03d}" for i in range(count)]
    new_current = current_id + count

    try:
        url = f"https://api.github.com/gists/{GIST_ID}"
        headers = {"Authorization": f"token {GITHUB_TOKEN}"}
        data = {"files": {"signal_counter.txt": {"content": str(new_current)}}}
...
def get_local_signal_ids(count=1):
...
def generate_chart(df, symbol, direction, entry, tp1, tp3, sl):
    ...
    ax.axhline(y=tp3, color='#27ae60', linestyle='--', linewidth=1.5, alpha=0.9, label=f'TP3: {tp3}')
...
def send_crypto_signal(signal_id, coin_name, direction, strategy, entry, tp1, tp2, tp3, sl, summary_text, chart_buf=None, strength_score=0):
    ...
    text = (
        ...
        f"COIN: #{clean_name}\n"
        f"Leverage: {LEVERAGE}\n"
        f"Direction: {direction_text} | Type: {strategy}\n"
        f"⭐ Signal Strength: {strength_score:.1f}/100\n"
        f"➖➖➖➖➖➖➖\n"
        f"ENTRY: {zone_low} - {zone_high}\n"
        f"TARGETS: {tp1} - {tp2} - {tp3}\n"
        f"STOP LOSS: {sl}\n"
        f"➖➖➖➖➖➖➖\n"
        f"📋 EXIT PLAN:\n"
        f"• Close 50% at TP1\n"
        ...
        f"📊 {summary_text}\n"
        f"➖➖➖➖➖➖➖\n"
        f"Crypto Bullets: By Banana Bot®"
    )
    ...
    if chart_buf:
        try:
            ...
            data = {"chat_id": CHANNEL_ID, "caption": f"📈 {clean_name} 4H Chart Analysis"}
...
def analyze_and_trade():
    ...
    signal_ids = get_next_signal_ids(len(top_signals))
